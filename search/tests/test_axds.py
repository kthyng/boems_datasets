...
#     reader = axdsReader()
    ...
    assert isinstance(station.data[dataset_ids], pd.DataFrame)
    assert (list(station.data[dataset_ids].index[[0,-1]].sort_values().values) == 
...
#     reader = axdsReader()
    ...
    stations = reader.stations(axds_type='platform2', dataset_ids=dataset_ids, kw=kw)
...
#     reader = axdsReader()
    ...
    stations = reader.stations(axds_type='platform2', stations=stationname, kw=kw)
...
#     reader = axdsReader()
    ...
    station = reader.stations(axds_type='layer_group', stations=stations[0])
    assert station.kw == {'min_time': '1900-01-01', 'max_time': '2100-12-31'}
...
#     reader = axdsReader()
    ...
    # SFBOFS module uuid
    dataset_ids = ['03158b5d-f712-45f2-b05d-e4954372c1ce']
    station = reader.stations(axds_type='layer_group', stations=stations, kw=kw)
...
#     reader = axdsReader()
    ...
    variables = ['Salinity']
    region = reader.region(kw=kw, variables=variables)
